...
public class Parser {
	   ...
	   void match(int t) throws IOException {  //匹配
	      if( look.tag == t ) move();
	   ...
	   public void program() throws IOException {  // program -> block
	      ...
	      // only display the stmts, without expr   只输出语句，不输出表达式
	      s.display();
	   }

	   Stmt block() throws IOException {  // block -> { decls stmts }  语句必须以  '{' 开始以 '}' 结束
	      match('{');  
	      Env savedEnv = top;   
	      top = new Env(top);
	      decls(); 
	   ...
	   Type type() throws IOException {  //判断 look 的类型
