...
Lexer::Lexer(string filepath)  //  filepath 可以是文本文件路径也可以是sql语句
{
	...
	words["primary"] = Word::primary;
	words["key"] = Word::key;
	words["use"] = Word::use;
	words["where"] = Word::where;
	words["\'"] = Word::sqm;
}
...
string Lexer::getATerm()
{
	...
	int QuotationMark = 0;  // 处理双引号个数
	if (peek == '"')
	{
		while(QuotationMark<2)
		{ 
		  term += peek;
		  if(peek=='"')
			  QuotationMark++;
...
bool Lexer::IsAIdentifier(string term)
{
	...
	for (int i = 0; i < term.length(); i++)
	{
		if (term.at(i) != '_') { ALLisUnderline = false;}
		if (term.at(i) == '"') return false;
	}
...
Word Lexer::scan()
{
	...
	else if (term.length()>=2 && term.at(0) == '"'&&term.at(term.length()-1) == '"')
	{
		term = term.substr(1, term.size() - 2);
		return Word(term, Tag::DQMCONTENT);  
	}
